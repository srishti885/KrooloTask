import { BsStars, BsPencilSquare, BsMagic, BsFilePdf, BsSearch, BsPlayCircle } from 'react-icons/bs';
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';

const aiTools = [
  { name: 'Grammar Checker', description: 'Easy to use checks', icon: BsPencilSquare, color: 'text-purple-600' },
  { name: 'Paraphraser', description: 'Improve content quality', icon: BsMagic, color: 'text-sky-600' },
  { name: 'Summarizer', description: 'Summarize content efficiently', icon: BsStars, color: 'text-amber-600' },
  { name: 'Chat with PDF', description: 'Real-time PDF chat', icon: BsFilePdf, color: 'text-rose-600' },
];
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        {aiTools.map((tool) => (
          ...
          >
            <div className={`p-3 rounded-2xl bg-gray-100 group-hover:bg-white transition-colors ${tool.color}`}>
              <tool.icon size={24} />
            </div>
            <div>
      ...
      <div className="mt-4 border-t border-gray-100 pt-4 px-2 flex justify-between items-center bg-gray-50/50 -m-4 -mb-4 p-4">
        ...
        {/* Instruction Text */}
        <span className="text-sm text-gray-600 font-medium">Try our AI tools now!</span>
